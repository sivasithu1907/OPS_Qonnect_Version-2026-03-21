                ...
                <div className={`bg-white rounded-2xl shadow-2xl w-full ${isMobile ? 'h-full rounded-none' : 'max-w-4xl max-h-[85vh]'} overflow-hidden relative flex flex-col md:flex-row`}>
                    ...
                    <div className="w-full md:w-1/3 bg-slate-50 p-8 border-r border-slate-100 flex flex-col items-center overflow-y-auto shrink-0">
                        ...
                        <div className="w-full space-y-3">
                            ...
                            {/* WhatsApp quick action (Sprint 2.3) */}
                            {activeItem.phone && (() => {
                              const n = activeItem.phone.replace(/[\s\-()]/g, '').replace(/^\+/, '');
                              return n ? (
                                <a href={`https://wa.me/${n}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-slate-700 p-2.5 bg-white rounded-xl shadow-sm border border-slate-100 active:bg-emerald-50 transition-colors">
                                  <div className="p-2 bg-emerald-50 rounded-lg text-emerald-600">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.113.553 4.094 1.516 5.814L.057 23.999l6.345-1.44A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.891 0-3.66-.519-5.17-1.418l-.371-.221-3.842.872.875-3.751-.236-.381A9.944 9.944 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
                                  </div>
                                  <div className="flex-1">
                                    <div className="text-xs text-slate-400 font-bold uppercase">WhatsApp</div>
                                    <span className="text-sm text-slate-800">Message on WA</span>
                                  </div>
                                  <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg">WA</span>
                                </a>
                              ) : null;
                            })()}
                            {activeItem.email && (

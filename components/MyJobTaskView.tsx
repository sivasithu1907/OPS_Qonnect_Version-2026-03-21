...
export const MyJobTaskView: React.FC<MyJobTaskViewProps> = ({ ticket, onUpdateStatus, onSelect }) => {
  ...
  return (
    <>
      {/* ── Full-screen detail view (matches Activity style) ── */}
      <div className="flex flex-col h-full overflow-y-auto bg-slate-50" onClick={() => onSelect?.(ticket)}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-white rounded-xl p-4 border border-slate-100 space-y-3">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className={`font-bold ${ticket.priority === 'URGENT' ? 'text-red-600' : ticket.priority === 'HIGH' ? 'text-orange-500' : 'text-slate-600'}`}>
                {ticket.priority}
              </span>
            </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm"
          ...
          <div className="bg-white w-full max-w-md rounded-t-[2rem] p-6 shadow-2xl"
